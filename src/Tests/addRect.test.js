...
describe('Can Add a shape from Menu', () => {
  ...
  test('can create a rectangle both in the sidebar and the canvas', async () => {
    ...
    await page.waitForSelector('#root > .app > .sidebar > .sidebar__menu > button:nth-child(7)')
    await page.click('#root > .app > .sidebar > .sidebar__menu > button:nth-child(7)')
    ...
    const widthInListItem = await page.$eval('#shapeWidthInput',element=> element.value);
    const widthInCanvasItem = await page.$eval('.rect',element=> element.style.width);
  ...
  test('can do it without the app, and the sideBar component\'s rendering', async () => {
    ...
    const appRendersCount =  await page.evaluate(() => document.querySelector('#app-render').innerText)
    const canvasRendersCount =  await page.evaluate(() => document.querySelector('#canvas-render').innerText)
    const sideBarRendersCount =  await page.evaluate(() => document.querySelector('#sideBar-render').innerText)
    const sideBarListRendersCount =  await page.evaluate(() => document.querySelector('#sideBarList-render').innerText)
